...
def encode_prompt(task, template, train_samples, eval_sample, tokenizer, max_length, sfc=False, icl_sfc=False, generation=False, generation_with_gold=False, max_new_tokens=None):
    ...
    # if sfc or icl_sfc:
    #     encode_fn = template_inst.encode_sfc
    #     verbalize_fn = template_inst.verbalize_sfc
    # else:
    #     encode_fn = template_inst.encode
    #     verbalize_fn = template_inst.verbalize
    ...
    if "gpt" in tokenizer_name or "opt" in tokenizer_name or "llama" in tokenizer_name or "Llama" in tokenizer_name:
...
# def encode_prompt(task, template, train_samples, eval_sample, tokenizer,
#                          max_length, sfc=False, icl_sfc=False,
#                          generation=False, generation_with_gold=False,
#                          max_new_tokens=None):
#     """
#     Encode prompts with LLaMA-3-style chat formatting: <s>[INST] ... [/INST]
#     """

#     def wrap_inst(content):
#         return f"<s>[INST] {content.strip()} [/INST]"

#     # ========== 1. Prepare demonstrations ==========
#     train_prompts = [template.verbalize(sample, sample.correct_candidate).strip() for sample in train_samples]
#     train_prompt_str = task.train_sep.join(train_prompts).strip()

#     # ========== 2. Get template functions ==========
#     if sfc or icl_sfc:
...
#     if not generation:
#         verbalized_eval_prompts = [verbalize_fn(eval_sample, cand).strip() for cand in eval_sample.candidates]
#         unverbalized_len = len(tokenizer.encode(unverbalized_eval_prompt))
#         option_lens = [
#             len(tokenizer.encode(vp)) - unverbalized_len for vp in verbalized_eval_prompts
#         ]
#         if sfc:
#             full_prompts = verbalized_eval_prompts
#         else:
#             full_prompts = [f"{train_prompt_str}{task.train_sep}{vp}".strip() for vp in verbalized_eval_prompts]
#     else:
#         assert not sfc and not icl_sfc
#         if generation_with_gold:
#             gold = eval_sample.correct_candidate
#             verbalized = verbalize_fn(eval_sample, gold)
#             unverbalized_len = len(tokenizer.encode(unverbalized_eval_prompt))
#             option_lens = [len(tokenizer.encode(verbalized)) - unverbalized_len]
#             full_prompts = [f"{train_prompt_str}{task.train_sep}{verbalized}".strip()]
#         else:
#             option_lens = [0]
#             full_prompts = [f"{train_prompt_str}{task.train_sep}{unverbalized_eval_prompt}".strip()]

#     # ========== 4. Wrap with LLaMA chat-style [INST] ==========
#     inst_prompts = [wrap_inst(p) for p in full_prompts]

#     # ========== 5. Tokenize ==========
#     encodings = [tokenizer.encode(p, add_special_tokens=False) for p in inst_prompts]

#     # ========== 6. Truncate ==========
#     if generation and max_new_tokens is not None:
#         max_length = max_length - max_new_tokens

#     if any(len(e) > max_length for e in encodings):
#         logger.warning("Truncating input exceeding max_length")

#     encodings = [e[-max_length:] for e in encodings]  # left truncate
#     encodings = [[tokenizer.bos_token_id] + e for e in encodings]  # add <s>
